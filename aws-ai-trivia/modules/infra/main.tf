...
resource "aws_subnet" "this" {
  ...
  map_public_ip_on_launch = true # Ensures ALB consistently attaches to public subnets
  tags = {
    Name = "cwb-ecs-${each.key}"
  }
}
...
resource "aws_lb_listener" "this" {
  ...
  default_action {
    ...
    fixed_response {
      ...
      message_body = "I'm working, but the tasks are not"
      status_code  = "503"
    }
  }
}
...
resource "aws_iam_policy" "ecs_secrets_policy" {
  ...
  description = "Allow ECS tasks to retrive all secrets from Secrets Manager"
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [
      {
        ...
        Resource = "*" # This should be limited to required resources as best practice
      }
    ]
  })
}
